...
import { validatePriorityPatch } from "../../../../../lib/admin-orders";
...
/**
 * PATCH /admin/api/orders/[id]/priority
 * Body JSON: { priority: Priority }
 */
export const PATCH: APIRoute = async ({ params, request, locals }) => {
  ...
  const validation = validatePriorityPatch(body);
  if (!validation.ok) {
  ...
  try {
    await pb.collection("orders").update(id, { priority: validation.priority });
  } catch (err: unknown) {
    return json(
      { ok: false, error: mapPBErrorToString(err, "Error al actualizar prioridad.") },
      500,
    );
  }

  return json({ ok: true, priority: validation.priority });
};
